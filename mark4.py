...
try:
    ...
    if messagebox.askyesno('ImportFix',
                           'There are missing dependencies needed for the program to work (module "num2words"). Would you like the program to install them?') == True:
        import subprocess
...
class Widget():
    ...
    def editWidget(self, index):
        ...
        try:
            ...
            edit.destroy()
            return
        instructions = tk.Label(edit, background='dark slate gray',
                                text='Once all answers have been added, select \na correct answer with the check marks beside them.\n[Only one answer can be selected]',
...
def finish_edits():
    ...
    with open(path + '/' + windowTitle + '.json', 'r') as toPlay:
        ...
        def checkifCorrect(ansNum):
            ...
            if ansNum == int(playData[questionplay[newx]][0]['Correct']):
                ...
                if len(questionplay) == 0:
                    ...
                    Ans3.destroy()
                    Ans4.destroy()
                    centeredQ.config(text='You got {0}\n\n{1} Correct, {2} Incorrect'.format(
                        str((correctans / (correctans + incorrectans)) * 100) + '%', correctans, incorrectans))
        ...
        centeredQ = tk.Label(ready, text=playData[questionplay[newx]][0]['Title'], relief='raised',
                             background='dark slate gray', fg='ivory')
...
def import_file():
    ...
    global Questions_Two, AddWidget, YforAdd, iter2
    name = filedialog.askopenfilename(initialdir=path, title='Select File',
